...
public class LcmConRecharge implements Processor {
	...
	public void process(Exchange exchange) throws Exception,JSONException {
		...
		JSONObject link_list_credit = (new JSONObject(exchange.getIn().getBody(String.class))).getJSONArray("relationship_list").getJSONObject(0).getJSONArray("link_list").getJSONObject(0).getJSONArray("records").getJSONObject(0).getJSONObject("link_value");
